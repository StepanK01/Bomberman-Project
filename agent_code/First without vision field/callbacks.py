...
def setup(self):
    ...
    self.agent = Agent(n_actions=len(ACTIONS), input_dims=self.input_dim, lr=0.0001, log=self.logger.debug) # lr=0.0001
...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    self.bomb_values.calc_value_field(agent_pos, crates_pos, other_agents_pos, walls_field, obstacle_field_reduced, danger_field, self.num_coins) #TODO eventuell nicht reduced
    ...
    if (features == np.zeros(22)).all():
        self.finders[3].last_recomendation = "WAIT"
    return features
...
def distance_from_pos(pos, target_pos):
    return np.abs(pos[0]-target_pos[0]) + np.abs(pos[1]-target_pos[1]) 
...
class bomb_place_values:
    ...
    def calc_value_field(self, pos, crates_pos, others_pos, walls_field, obstacle_field, dangerfield, num_coins):
        self.requested_num = 0
        self.dims = obstacle_field.shape
        self.pre_dist_value_field = self.get_crates_value_field(crates_pos, walls_field, num_coins) + self.get_others_value_field(pos, others_pos, walls_field, obstacle_field, dangerfield)
    ...
    def get_crates_value_field(self, crates_pos, walls_field, num_coins):
    ...
    def get_danger_of_other(self, pos, other_pos, bomb_pos, walls_field, obstacle_field, dangerfield):
        ...
        score = 10*np.exp(-len(dangerless_pos)/2)
        score = max(score, 1) 
...
class coins_direction_finder(direction_finder):
    ...
    def get_direction_from_path(self, path, path_len, field, others_finder:others_direction_finder, others_pos):
        ...
        path_len = path_len - coin_to_others_action[-1] 
